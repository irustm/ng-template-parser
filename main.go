...
func main() {
	...
	var gAst, gErr = spegParser.ParseGrammar(getNgGrammar())
	if !gErr {
		...
		if strings.Contains(data, "{{") {
			...
			for i := 0; i < len(data); i++ {
				if data[i] == '{' {
					...
					expressions = append(expressions, parseExpression(string(buff)))
					buff = nil
					i += 1
				} else {
...
func parseExpression(text string) rd.Ast {
